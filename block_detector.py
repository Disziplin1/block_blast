...
2. ROI 가장자리(배경)의 채도 평균/표준편차로 Adaptive Threshold 를 계산하고,
...
4. 보드 셀 크기 x tray.piece_cell_scale 로 추정한 셀 픽셀 크기를 이용해
   바운딩 박스를 NxM 그리드로 나누고, 각 서브셀의 안쪽(margin) 영역의
   마스크 비율/평균 색상으로 0/1 (Occupied) 을 결정한다.
"""
...
@dataclass
class DetectedPiece:
    grid: np.ndarray            # 0/1 2차원 배열 (최소 바운딩 박스 크기, Solver 입력용)
    ...
    # bbox 를 grid_rows x grid_cols 로 나눈 (trim 이전) 0/1 배열
...
class BlockDetector:
    ...
    def detect_slot(self, frame: np.ndarray, slot_rect: Tuple[int, int, int, int]) -> DetectedPiece:
        ...
        grid = self._trim_grid(debug_grid)
        if grid.sum() == 0:
            return DetectedPiece(
                grid=np.zeros((1, 1), dtype=np.int8), cells=tuple(), bbox=(0, 0, 0, 0), empty=True,
        ...
        return DetectedPiece(
            grid=grid, cells=cells, bbox=(bx, by, bw, bh), empty=False,
            debug_grid=debug_grid, debug_cells=debug_cells, threshold_used=threshold_used,
        )
